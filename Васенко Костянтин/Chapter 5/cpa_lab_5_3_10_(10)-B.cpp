...
public:
	...
	bool contains(Point2D *p1) {
		if (p1->y == ((this->slope * p1->x) + this->y_intersect)) {
			return true;
		}
...
int main() {
	...
	while (true)
	{
		...
		std::getline(std::cin, point3_input);

		istringstream iss3(point3_input);
		vector<string> tokens3;
		copy(istream_iterator<string>(iss3),
			istream_iterator<string>(),
			back_inserter(tokens3));
		Point2D p3(std::stod(tokens3[0]), std::stod(tokens3[1]));

		bool contains = line.contains(&p3);
